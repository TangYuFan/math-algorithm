...
/**
 *   @desc : weka RandomSubSpace随机子空间算法
 *   @auth : tyf
 *   @date : 2022-03-04  17:17:22
*/
public class WekaRandomSubSpace {

    // RandomSubSpace 随机子空间算法，通过随机选择特征子集进行训练。
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 模型保存和加载路径
        String module = "C:\\Users\\tyf\\Desktop\\WekaRandomSubSpace.model";
        ...
        RandomSubSpace subSpace = new RandomSubSpace();
        subSpace.setClassifier(new RandomTree()); // 设置基础二分类器
        subSpace.setNumIterations(50); // 迭代次数
        subSpace.setOptions(new String[]{"-I","30"}); // 随机树的个数、更多参数见函数注释


        // 训练
        subSpace.buildClassifier(trainingData);

        Evaluation evaluation = new Evaluation(validationData);
        evaluation.evaluateModel(subSpace, validationData);
        ...
        // 模型保存
        SerializationHelper.write(module, subSpace);
        RandomSubSpace model = (RandomSubSpace) SerializationHelper.read(module);
