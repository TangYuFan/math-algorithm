...
/**
 *   @desc : Smile XMeans 聚类
 *   @auth : tyf
 *   @date : 2022-03-08  16:38:14
*/
public class SmileXMeans {

    // 聚类
    // 是一种扩展的K均值聚类算法，旨在自动确定数据中的簇数量。与传统的K均值算法不同，
    // XMeans通过动态地调整簇的数量，使模型更灵活，能够适应数据中不同数量的簇。
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 模型路径
        String module = "C:\\Users\\tyf\\Desktop\\xMeans";
        ...
        int k = 5;
        XMeans xMeans = XMeans.fit(trainData.select("f1","f2","f3","f4","f5").toArray(),k);

        System.out.println("XMeans:"+xMeans);

        System.out.println("\n模型序列化：");
        Write.object(xMeans,new File(module).toPath());
        System.out.println("Write module："+module);
        XMeans obj = (XMeans)Read.object(new File(module).toPath());
