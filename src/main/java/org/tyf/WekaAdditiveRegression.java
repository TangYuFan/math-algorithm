...
/**
 *   @desc : weka AdditiveRegression加法回归
 *   @auth : tyf
 *   @date : 2022-03-04  17:17:22
*/
public class WekaAdditiveRegression {

    // AdditiveRegression 加法回归算法，用于回归问题。
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 模型保存和加载路径
        String module = "C:\\Users\\唐于凡\\Desktop\\regression.model";
        ...
        // 打印模型参数
        System.out.println("------打印模型参数------");
        System.out.println(regression);
        System.out.println("------打印模型参数------");

        // 使用验证集进行模型评估
        Evaluation evaluation = new Evaluation(validationData);
        evaluation.evaluateModel(regression, validationData);
        ...
        SerializationHelper.write(module, regression);
        AdditiveRegression model = (AdditiveRegression) SerializationHelper.read(module);
        // 使用模型进行单个样本预测
        for (int i = 0; i < 30; i++) {
