...
/**
 *   @desc : Smile MLPReg 神经网络回归
 *   @auth : tyf
 *   @date : 2022-03-08  16:38:14
*/
public class SmileMLPReg {
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 几个预处理函数
        // WinsorScaler.fit(); 指定的范围以原始分布的百分位数表示
        // MaxAbsScaler.fit(); 按每个特征的最大绝对值进行缩放
        // Standardizer.fit(); 将数字特征转换为 0 均值和单位方差
        // Normalizer; 将样本转换为单位范数

        // 将指定列进行标准化,缩放到0~1
        InvertibleColumnTransform transform = MaxAbsScaler.fit(df,"f1","f2","f3","f4","score");

        DataFrame ndf = transform.apply(df);
        ...
        // 划分训练集和验证集
        double radio = 0.98;
        int c = Double.valueOf(ndf.size()*radio).intValue();

        // 训练样本和验证样本
        DataFrame trainData = ndf.slice(0,c);
        DataFrame valiaData = ndf.slice(c,ndf.size());

        System.out.println("trainData："+trainData.size());
        System.out.println("valiaData："+valiaData.size());

        Properties params = new Properties();
        params.setProperty("smile.mlp.layers", "ReLU(10)");
        params.setProperty("smile.mlp.epochs", "10");
        params.setProperty("smile.mlp.mini_batch", "4");
        ...
        MLP fit = MLP.fit(in_data, la_data, params);
