...
/**
 *   @desc : Smile OneVersusRest 1vAll分类器
 *   @auth : tyf
 *   @date : 2022-03-08  16:38:14
*/
public class SmileOneVersusRest {
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 模型路径
        String module = "C:\\Users\\tyf\\Desktop\\SmileOneVersusRest";
        ...
        // 基础分类器必须是二分类器,底层会调用 PlattScaling.fit 函数
        DataFrameClassifier oneVersusRest = OneVersusRest.fit(
                formula,
                trainData,
                (f, b) -> AdaBoost.fit(f,b)
        );

        System.out.println("numClasses："+oneVersusRest.numClasses());
        System.out.println("classes："+Arrays.toString(oneVersusRest.classes()));


        // 模型评估
        int[] truthIndex = valiaData.intVector("labelIndex").stream().toArray(); // 样本 label 索引
        int[] prediIndex = oneVersusRest.predict(valiaData); // 预测 label 索引
//
//        System.out.println("\n模型评估：");
//        System.out.println("Accuracy："+ Accuracy.of(truthIndex,prediIndex));
//
//        System.out.println("\n模型序列化：");
//        Write.object(oneVersusRest,new File(module).toPath());
//        System.out.println("Write module："+module);
//        DataFrameClassifier obj = (DataFrameClassifier)Read.object(new File(module).toPath());
