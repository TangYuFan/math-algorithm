...
public class SparkNaiveBayes {
    ...
    // 高斯朴素贝叶斯（Gaussian Naive Bayes）： 用于连续型数据，假设每个特征的值都服从正态分布。
    // 多项式朴素贝叶斯（Multinomial Naive Bayes）： 用于离散型数据，特别适用于文本分类问题，其中特征表示单词的计数。
    // 伯努利朴素贝叶斯（Bernoulli Naive Bayes）： 也用于二值型特征，通常用于文本分类问题，其中特征表示单词的出现与否。
    ...
    // spark-submit --master spark://172.16.5.248:7077 --class org.tyf.SparkNaiveBayes /usr/algorithm-v1-jar-with-dependencies.jar
    ...
    public static void main(String[] args) {
        ...
        SparkConf conf = new SparkConf().setAppName("SparkNaiveBayes").setMaster("local[*]");
        ...
        // 获取训练好的随机森林模型
        NaiveBayesModel rfModel = (NaiveBayesModel) (model.stages()[2]);
        System.out.println("NaiveBayesModel:\n" + rfModel);
