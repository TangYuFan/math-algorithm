...
/**
 *   @desc : Smile GLMReg 广义线性模型回归
 *   @auth : tyf
 *   @date : 2022-03-08  16:38:14
*/
public class SmileGLMReg {
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 模型路径
        String module = "C:\\Users\\tyf\\Desktop\\SmileGLMReg";
        ...
        Formula formula = Formula.of("score","f1","f2","f3","f4");

        Model model = Poisson.log();
//        Model model = Binomial.logit();
//        Model model = Bernoulli.logit();

        GLM glm = GLM.fit(formula,trainData,model);


        // 模型评估 - 准确率
        double[] truth = valiaData.doubleVector("score").toDoubleArray(); // 真实值
        double[] predit = glm.predict(valiaData);
        ...
        System.out.println("\n模型序列化：");
        Write.object(glm,new File(module).toPath());
        System.out.println("Write module："+module);
        GLM obj = (GLM)Read.object(new File(module).toPath());
        ...
        for (int i = 0; i < 30; i++) {
            ...
            double out = obj.predict(valiaData.of(i))[0]; // 预测值
        ...
        lines.canvas().setTitle("RegressionTreeReg");
        lines.canvas().window();
