...
/**
 *   @desc : spark One-vs-Rest 分类
 *   @auth : tyf
 *   @date : 2022-02-22  15:10:34
 */
public class SparkOneVsRest {


    // One-vs-Rest 算法
    // One-vs-Rest（OvR），也称为One-vs-All（OvA），是一种多类别分类的策略。该算法的主要作用是将多类别分类问题转化为多个二分类问题，从而使用二分类分类器进行处理。它的基本思想是为每个类别训练一个二分类器，该分类器将该类别与所有其他类别进行区分。
    ...
    // spark-submit --master spark://172.16.5.248:7077 --class org.tyf.SparkOneVsRest /usr/algorithm-v1-jar-with-dependencies.jar
    ...
    public static void main(String[] args) {
        ...
        SparkConf conf = new SparkConf().setAppName("SparkOneVsRest").setMaster("local[*]");
        ...
        // 模型
        OneVsRestModel rfModel = (OneVsRestModel) (model.stages()[2]);
        System.out.println("OneVsRestModel:\n" + rfModel);
